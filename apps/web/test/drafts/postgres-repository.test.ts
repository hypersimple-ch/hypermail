...
const draftRow = (state: 'sent' | 'failed'): SqlRow => ({
  ...
  created_at: '2025-01-01T00:00:00.000Z', updated_at: '2025-01-01T00:01:00.000Z', recipients: [{ kind: 'to', address: 'person@example.com' }], subject: 'Hello', body: 'Body',
});
const claim: ApprovalClaim = {
  ...
  draft: { ...draftRow('sending'), id: draftId, accountId, sourceMessageId: null, createdBy: 'user', state: 'sending', version: 1, createdAt: '2025-01-01T00:00:00.000Z', updatedAt: '2025-01-01T00:00:00.000Z', recipients: [{ kind: 'to', address: 'person@example.com' }], subject: 'Hello', body: 'Body' },
};
...
describe('PostgresDraftRepository send completion', () => {
  ...
  it.skipIf(!process.env.DATABASE_URL)('runs completion and reused confirmation text against migrated PostgreSQL FKs', async () => {
    ...
    try {
      ...
      const input = { accountId: seededAccountId, createdBy: 'user' as const, recipients: [{ kind: 'to' as const, address: 'person@example.com' }], subject: 'Hello', body: 'Body' };

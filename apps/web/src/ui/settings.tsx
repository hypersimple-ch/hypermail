...
function PendingConnection({ pending, disabled, onComplete }: { pending: PendingMailboxConnection; disabled: boolean; onComplete: (input: CompleteMailboxConnectionInput) => void }) {
  ...
  return <Card className="gap-0 py-0" aria-label={`${isOutlook ? 'Outlook' : 'Gmail'} connection`}> 
    ...
    <CardContent className="space-y-4 px-4 pb-4">
      {pending.authorizationUrl ? <Link href={pending.authorizationUrl} {...(isOutlook ? { target: '_blank', rel: 'noreferrer' } : {})} className="inline-flex min-h-11 items-center gap-2 text-sm font-medium underline underline-offset-4">Open {isOutlook ? 'Microsoft verification' : 'Google sign-in'} <ExternalLink aria-hidden="true" className="size-4" /></Link> : null}

...
async function body(request: IncomingMessage): Promise<Readonly<Record<string, unknown>>> { const chunks: Uint8Array[] = []; let size = 0; for await (const chunk of request) { const value = Buffer.from(chunk); size += value.length; if (size > 8_192) throw new RangeError('Payload too large'); chunks.push(value); } if (!chunks.length) return {}; const raw=Buffer.concat(chunks); if (request.headers['content-type']?.split(';')[0]?.trim() === 'application/x-www-form-urlencoded') { const params=new URLSearchParams(raw.toString('utf8')); const result: Record<string,string>={}; for(const [key,value] of params){ if(key.length>128||value.length>4096||key in result) throw new TypeError('Invalid form'); result[key]=value; } return result; } try { return JSON.parse(raw.toString('utf8')) as Readonly<Record<string, unknown>>; } catch { return {}; } }
...
async function handleRequest(request: IncomingMessage, response: ServerResponse, throttle: RequestThrottle, id: string, runtime?: WebRuntime): Promise<void> {
  ...
  // /mcp owns the raw IncomingMessage so its 512 KiB JSON limit and SDK streaming semantics
  // are not affected by the generic 8 KiB API parser.

...
describe('worker health contract', () => {
  ...
  it('reports every unavailable operational dependency without details', () => {
    expect(readiness({ database: false, queue: false, hypermail: true, scheduler: false, model: true, notifications: true, policy: true })).toEqual({
      status: 'not_ready',
      dependencies: ['database', 'queue', 'scheduler'],
    });
  });

  it('reports ready when every required runtime capability is available', () => {
    expect(readiness({ database: true, queue: true, hypermail: true, scheduler: true, model: true, notifications: true, policy: true })).toEqual({
      status: 'ready',
      dependencies: [],
    });
  });

...
describe('automatic Task adapters',()=>{
  it('external claims are pinned to the authenticated connection and forward fenced results',async()=>{
    ...
    await protocol.claim('00000000-0000-4000-8000-000000000001','worker');
    expect(claim).toHaveBeenCalledWith({kind:'agent_connection',connectionId:'00000000-0000-4000-8000-000000000001'},'worker');
  });
  it('Mastra asks only for the mastra target and never falls back to an external claim',async()=>{
    ...
    expect(await adapter.runOne()).toBe(false);
    expect(claim).toHaveBeenCalledTimes(1);
    expect(claim).toHaveBeenCalledWith({kind:'mastra'},'embedded');
  });
});

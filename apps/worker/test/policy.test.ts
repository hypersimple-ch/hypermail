...
describe('worker policy boundary', () => {
  it('uses exact allowlisted tool names and provider identities, never app UUIDs', async () => {
    ...
    const db = database(async sql => ({ rows: sql.includes('app.folders') ? [{ providerFolderId: 'provider-folder' }] : [{ email: 'account@example.test', providerMessageId: 'provider-message' }] }));
    const transport = new HypermailPrivateMutationTransport(db, { call: async (name, args) => { calls.push({ name, args }); return {}; } }, () => Promise.resolve());
    ...
    await expect(transport.draftCreate({ target: { accountId: ids.account, draftId: ids.folder }, idempotencyKey: 'x'.repeat(16) })).rejects.toThrow('POLICY_DRAFT_RESPONSE_UNVERIFIED');
  });
  ...
  it('does not fabricate folder verification facts', async () => {
    ...
    const transport = new HypermailPrivateMutationTransport(database(async () => ({ rows: [{ email: 'a@example.test', providerMessageId: 'provider' }] })), { call: async name => name === 'read_email' ? { isRead: true } : {} }, () => Promise.resolve());

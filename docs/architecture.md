...
Hypermail is a private, single-user, Android-first PWA. Only the HTTPS proxy and web service are internet reachable. PostgreSQL, the worker, and Hypermail MCP remain on a private service network. Provider credentials and OAuth tokens stay in Hypermail encrypted state; the application database stores account projections only.
...
flowchart LR
  ...
  K -->|message text only; no attachment bytes| M[Configured model API]
...
Mastra source history is partitioned by stable account resource IDs and stable activity threads, with a separate global-constraints resource. The production triage workflow passes that resource/thread to an Agent configured with Observational Memory, validates structured output again at the application boundary, and atomically persists deterministic decision/question attempts before any policy work. Typed questions suspend in PostgreSQL and resume idempotently after restart. Derived Observational Memory is deliberately opaque in milestone one because the pinned public API cannot reliably inspect, correct, or reset it. No UI claims otherwise.
...
Body purging is bounded, leased, audited, and requires both the configured cache-age cutoff and the row's `purge_after`; it does not cascade to messages, activities, decisions, actions, notifications, or audits. Expired push subscriptions are disabled, not removed. Attachments are metadata-only in PostgreSQL; bytes are streamed on explicit request and never retained intentionally. Web startup must finish bounded owned-orphan cleanup before listening. Deployment gives web and Hypermail one mode-0700 named temp volume and common non-root UID; no other service receives that volume.

## Backup, observability, and security

The one-shot private backup job produces separate age-encrypted PostgreSQL custom dumps and Hypermail-state archives, uploads immutable generations and an encrypted integrity manifest to an off-host S3 prefix, prunes only exact owned generation artifacts, emits sanitized failure alerts, and is scheduled daily by the deployment platform. Database and state identities are distinct restricted secret files. Restore requires an explicit isolation flag, a database named only `restore` or `hypermail_restore`, an empty state target, matching manifest hashes/sizes, and successful decryption before mutation. The seeded disposable restore drill and measured evidence are release inputs; real Hypermail maintenance-mode/state recovery remains part of provider acceptance.

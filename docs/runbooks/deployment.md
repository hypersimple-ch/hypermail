...
2. Select and pin an approved Hypermail **v0.7.26** digest in `HYPERMAIL_IMAGE`; prove its state path, protocol contract, and UID/GID 10001 behavior in the proposed deployment. Web and Hypermail share that identity only for the mode-0700 attachment volume.
...
4. Keep provider OAuth/IMAP credentials exclusively in `HYPERMAIL_ENV_FILE`; do not put them in web or worker files. Configure DNS/firewall for TCP 80/443 only; never publish web 3000, worker health 3001, PostgreSQL, or Hypermail.

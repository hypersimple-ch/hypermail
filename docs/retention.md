...
Mailbox-memory events retain stable UUID/source identity, tenant ownership, content digests, delivery counters/timing, and sanitized result/error metadata. Content required for replay remains in `content_payload` while an event is pending, processing, or deferred, and automatic delivery retries indefinitely with capped backoff. There is no attempt limit or dead-letter state. Successful completion atomically sets that duplicate payload to `NULL`. Permanent bank deletion leaves completed canonical history unchanged, while pending and processing events become terminal `cancelled` rows with replay payload removed and the sanitized `OWNER_MEMORY_DELETION` outcome. The append-only guard permits only completion or cancellation minimization and never permits row deletion. Attachment bytes are never stored in the event table, audits, or logs. Hindsight bank retention and permanent deletion are separate projection-lifecycle operations; disconnect does not make this canonical outbox disposable.

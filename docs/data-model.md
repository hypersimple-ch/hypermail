...
V1 writes learning events only as part of new canonical transitions. There is no historical backfill. The canonical transaction enqueues question answers; User-authored draft creation and revisions; explicit User correction, confirmation, or rejection of agent drafts; authoritative send outcomes; and verified, failed, or unverifiable organize, read/unread, and draft actions. Agent-only draft creation or editing is not learned before User feedback. Stale, conflicting, replayed, cancelled, or unchanged paths do not enqueue a new event. A failed or unverifiable action uses a distinct outcome kind and is never represented as verified.

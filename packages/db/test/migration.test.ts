...
describe('owner send approval migration',()=>{
  it('replaces the state enum atomically so batched pending migrations can use every value',()=>{expect(journal).toContain('0010_owner_send_approval');expect(ownerSendSql).not.toContain('ADD VALUE');expect(ownerSendSql).toContain('RENAME TO public_send_request_state_legacy');expect(ownerSendSql).toContain("CREATE TYPE app.public_send_request_state AS ENUM('pending_owner_approval','expired','cancelled','approved','rejected','sending','failed','unverifiable')");expect(ownerSendSql).toContain('ALTER COLUMN state TYPE app.public_send_request_state');expect(ownerSendSql).toContain("pending_owner_approval' AND NEW.state IN ('expired','cancelled','rejected','sending')");expect(ownerSendSql).toContain("OLD.state='unverifiable' AND NEW.state IN ('approved','failed')");});
